...
def tool_search_kb(query: str, top_k: int = 3) -> dict:
    ...
    try:
        ...
        chunks = [{"text": c["text"], "source": c["metadata"].get("source", "unknown"), "score": c["score"]} for c in raw[:top_k]]
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="MCP Server: mock + FastAPI HTTP server")
    parser.add_argument("--http", action="store_true", help="Run FastAPI HTTP server on port 8000")
    ...
    if args.http:
        ...
        print("\nChạy: python mcp_server.py --http để bật HTTP server (FastAPI). Bonus +2!")
